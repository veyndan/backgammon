	...
	#legalTurnsSub(pml, anRoll, nMoveDepth, iPip, cPip, anBoard, anMoves, fPartial) {
		...
		if (anBoard.mailbox[1][24]) {
			...
			for (i = iPip; i >= 0; i--) {
				...
				// console.log(anBoard.mailbox);
				// console.log(`hello ${i} ${anBoard.mailbox[1][i]} ${this.#LegalMove(anBoard, i, anRoll[nMoveDepth])}`);
...
export function divide(player, board, maxDepth) {
	...
	for (let die0 = 1; die0 <= 6; die0++) {
		for (let die1 = die0; die1 <= 6; die1++) {
			...
			console.log(`${die0}-${die1}`, legalTurns.cMoves);
	...
	console.log(`Nodes → ${totalTurnCount}`);
	return {
