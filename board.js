...
export class Board {
	...
	#SaveMoves(pml, cMoves, cPip, anMoves, anBoard) {
		console.log(cMoves, cPip, 0, pml.cMaxMoves, pml.cMaxPips);
		...
		for (let i = 0; i < pml.cMoves; i++) {
			process.stdout.write("alsdf ");
			let amMove = pml.amMoves[i];
			for (let j = 0; j < amMove.cMoves; j++) {
				process.stdout.write(`${amMove.anMove[j]} `);
			}
		...
		for (let i = 0; i < pml.cMoves; i++) {
			...
			console.log(`${pml.cMoves} ${pm.cMoves}`);
			console.log(key.data.join(` `) + ` x`);
			console.log(pm.key.data.join(` `) + ` y`);
		...
		console.log(pm.key.data.join(` `) + ` z`);

		console.log(`blah ${cMoves}`);
		pm.cMoves = cMoves;
		pm.cPips = cPip;
	...
	#legalTurnsSub(pml, anRoll, nMoveDepth, iPip, cPip, anBoard, anMoves) {
		console.log(`${nMoveDepth} ${iPip} ${cPip} 0`);
		/** @type {boolean} */
		let fUsed = false;
		/** @type {Board} */
 ...
 * @return {number}
 */
export function divide(board, maxDepth) {
	let totalTurnCount = 0;
	for (let die0 = 5; die0 <= 5; die0++) {
		for (let die1 = 6; die1 <= 6; die1++) {
